...
public class PipelineManager {
    ...
    public void launchPipeline(PipelineConfiguration config) {
        ...
        for (PipelineTask existingTask : runningTasks) {
            if (existingTask.getDescription().equals(taskDescription)) {
                JOptionPane.showMessageDialog(parentComponentForDialogs,
                        "Uma pipeline para '" + taskDescription + "' já está em execução.",
                        "Pipeline Duplicada",
                        JOptionPane.WARNING_MESSAGE);
                logger.warn("Tentativa de iniciar uma pipeline duplicada para '{}' foi bloqueada.", taskDescription);
