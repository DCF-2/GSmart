...
public class TaskManagerWindow extends JFrame {
    ...
    public TaskManagerWindow(PipelineManager pipelineManager) {
        ...
        // Define o Runnable que atualiza a tabela
        Runnable tableUpdater = () -> tableModel.updateTasks(this.pipelineManager.getRunningTasks());

        // Adiciona o Runnable como um ouvinte
        this.pipelineManager.addTaskListUpdatedListener(tableUpdater);

        // Garante que o ouvinte é removido quando a janela fecha para evitar memory leaks
     ...
     * @param newTasks A nova lista de tarefas ativas.
     */
    public void updateTasks(List<PipelineTask> newTasks) {
        if (tableModel != null) {
            tableModel.updateTasks(newTasks);
        }
    }
