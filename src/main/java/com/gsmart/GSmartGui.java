...
import main.java.com.gsmart.config.*;
...
import main.java.com.gsmart.sources.*;

// Imports das configuraçoes de pipelines
...
public class GSmartGui extends JFrame {
    ...
    public GSmartGui(LogViewerWindow logViewer, PipelineManager pipelineManager, String userRole) {
        ...
        try {
            // MÉTODO MAIS ROBUSTO: Usa ImageIO para ler o stream do recurso
            Image icon = ImageIO.read(getClass().getResource("/gsmart_icon.png"));
            setIconImage(icon);
        } catch (Exception e) {
            logger.error("Ficheiro do ícone (gsmart_icon.png) não foi encontrado no classpath (verifique 'target/classes').", e);
        }
        ...
        gbcTb.gridx = 0;
        gbcTb.gridy = 0;
        thingsboardConfigPanel.add(new JLabel("URL do Servidor:"), gbcTb);
        gbcTb.gridx = 1;
        thingsboardUrlField = new JTextField();
        gbcTb.weightx = 1.0;
        gbcTb.fill = GridBagConstraints.HORIZONTAL;
        thingsboardConfigPanel.add(thingsboardUrlField, gbcTb);
        gbcTb.gridx = 2;
        tbConnectButton = new JButton("Conectar");
        gbcTb.weightx = 0;
        gbcTb.fill = GridBagConstraints.NONE;
        thingsboardConfigPanel.add(tbConnectButton, gbcTb);
        gbcTb.gridx = 3;
        tbStatusLabel = new JLabel("Não conectado");
        tbStatusLabel.setForeground(Color.GRAY);
        thingsboardConfigPanel.add(tbStatusLabel, gbcTb);
        gbcTb.gridx = 0;
        gbcTb.gridy = 1;
        ...
        gbcTb.gridwidth = 3;
        gbcTb.fill = GridBagConstraints.HORIZONTAL;
        deviceProfileSelector.setEnabled(false);
        ...
        gbcDb.gridy = 0;
        gbcDb.gridwidth = 1;
        databaseConfigPanel.add(new JLabel("URL do Banco (JDBC):"), gbcDb);
        gbcDb.gridx = 1;
        gbcDb.gridy = 0;
        gbcDb.gridwidth = 3;
        gbcDb.weightx = 1.0;
        ...
        databaseConfigPanel.add(dbUrlField, gbcDb);
        gbcDb.gridx = 4;
        gbcDb.gridy = 0;
        gbcDb.gridwidth = 1;
        ...
        gbcDb.gridy = 1;
        gbcDb.gridwidth = 1;
        databaseConfigPanel.add(new JLabel("Usuário:"), gbcDb);
        gbcDb.gridx = 1;
        gbcDb.gridy = 1;
        gbcDb.gridwidth = 3;
        gbcDb.weightx = 1.0;
        ...
        databaseConfigPanel.add(dbUserField, gbcDb);
        gbcDb.gridx = 0;
        gbcDb.gridy = 2;
        gbcDb.gridwidth = 1;
        databaseConfigPanel.add(new JLabel("Senha:"), gbcDb);
        gbcDb.gridx = 1;
        gbcDb.gridy = 2;
        gbcDb.gridwidth = 3;
        gbcDb.weightx = 1.0;
        ...
        databaseConfigPanel.add(dbPasswordField, gbcDb);
        gbcDb.gridx = 0;
        gbcDb.gridy = 3;
        gbcDb.gridwidth = 1;
        databaseConfigPanel.add(new JLabel("Tabela:"), gbcDb);
        gbcDb.gridx = 1;
        gbcDb.gridy = 3;
        gbcDb.gridwidth = 3;
        gbcDb.weightx = 1.0;
        ...
        loadKeysButton.setVisible(false); // Desativado, pois ao clicar no  conectar ja deve carregar as metricas.
        ...
        runInBackgroundCheckBox = new JCheckBox("Executar em 2º plano ao fechar");
        runInBackgroundCheckBox.setToolTipText("Se marcado, o GSmart continuará a correr na bandeja do sistema ao invés de fechar.");
        ...
        autoStartButton.addActionListener(e -> {
            AutoStartManagerWindow autoStartManager = new AutoStartManagerWindow(this, configManager);
            autoStartManager.setVisible(true);
        });
        ...
        adminPanel.add(runInBackgroundCheckBox);
        ...
        adminPanel.add(new JSeparator(SwingConstants.VERTICAL)); // Separador visual
        ...
        deviceProfileSelector.addItemListener(e -> {
            if (e.getStateChange() == ItemEvent.SELECTED) loadDevicesByProfile();
        });
        // Apenas aciona a ação quando um novo item é selecionado
        deviceSelector.addItemListener(e -> {
            if (e.getStateChange() == ItemEvent.SELECTED) {
                loadAvailableKeys();
            }
        });
        // Listener para o seletor de Tabela (Base de Dados)
        dbTableSelector.addItemListener(e -> {
            if (e.getStateChange() == ItemEvent.SELECTED) {
                loadAvailableKeys();
            }
        });
        ...
        dashboardPanel.getOpenTaskManagerButton().addActionListener(e -> showTaskManager());

        // Callback para atualizar o Dashboard sempre que a lista de tarefas mudar
        this.pipelineManager.addTaskListUpdatedListener(() -> {
            int taskCount = this.pipelineManager.getRunningTasks().size();
            dashboardPanel.updateActivePipelinesCount(taskCount);
        });
        ...
        thingsboardUrlField.getDocument().addDocumentListener(createRealtimeValidator(thingsboardUrlField, urlValidator));
        pbiUrlField.getDocument().addDocumentListener(createRealtimeValidator(pbiUrlField, urlValidator));
        java.util.function.Predicate<String> tcpValidator = s -> s.toLowerCase().startsWith("tcp://");
        mqttBrokerUrlField.getDocument().addDocumentListener(createRealtimeValidator(mqttBrokerUrlField, tcpValidator));
        java.util.function.Predicate<String> fabricValidator = s -> s.contains("Endpoint=");
        fabricConnectionStringField.getDocument().addDocumentListener(createRealtimeValidator(fabricConnectionStringField, fabricValidator));

        // Listener da Janela
        addWindowListener(new WindowAdapter() {
            @Override
            public void windowClosing(WindowEvent e) {
                if (runInBackgroundCheckBox.isSelected()) {
                    setVisible(false);
                    return; // Interrompe a execução para não fechar a aplicação
                }
                handleWindowExit();
            }
        });
    ...
    private void saveConfiguration() {
        ...
        configManager.saveMetricConfigs(tableModel.getAllMetrics());
    ...
    private void launchPipeline() {
        ...
        try {
            // Obtém e valida a URL do Power BI a partir do campo de texto.
        ...
        new SwingWorker<List<String>, Void>() {
            ...
            protected void done() {
                try {
                    ...
                    // Se houver tabelas, carrega as métricas da primeira tabela automaticamente.
                    if (!tables.isEmpty()) {
                        loadAvailableKeys();
                    }
                } catch (Exception e) {
    ...
    private void handleKeysLoaded(SwingWorker<List<String>, Void> worker) {
        try {
            ...
            // 1. Carrega as configurações de métricas que o utilizador já salvou.
            List<MetricConfig> savedConfigs = configManager.loadMetricConfigs();
            ...
            // 2. Cria a nova lista de métricas, aplicando as configurações salvas.
            List<MetricConfig> newConfigs = keys.stream().map(key -> {
                // Se existir uma configuração salva para esta chave, usa-a.
                if (savedConfigMap.containsKey(key)) {
                    return savedConfigMap.get(key);
                }
                // Caso contrário, cria uma nova configuração padrão.
                return new MetricConfig(key);
            }).collect(Collectors.toList());

            // 3. Adiciona as métricas de sistema (sempre no topo e sem serem afetadas pelas salvas)
            ...
            // 4. Define a lista final e inteligente na tabela.
            tableModel.setMetrics(newConfigs);
        ...
        new SwingWorker<List<Device>, Void>() {
            ...
            protected void done() {
                try {
                    ...
                    // Se houver dispositivos, carrega as métricas do primeiro dispositivo automaticamente.
                    if (!devices.isEmpty()) {
                        loadAvailableKeys();
                    }
     ...
     */
    private javax.swing.event.DocumentListener createRealtimeValidator(JTextField textField, java.util.function.Predicate<String> validator) {
